...
const USER_EMAIL    = 'Playwright2026@gmail.com';
const USER_PASSWORD = 'Password@2026'; 
...
test ('create event via UI, book it, and verify seat reduction', async ({ page }) => {
  ...
  const eventTitle = `Tech Mela ${Date.now()}`;

  // Located by id (explicit on the component)
  await page.locator('#event-title-input').fill(eventTitle);

  // Description — only textarea in the form
  await page.locator('#admin-event-form textarea').fill('Tech Mela for all Technologies');

  // Located by label (Select auto-generates id from label text)
  await page.getByLabel('City').fill('Panaji');
  await page.getByLabel('Venue').fill('Banquete');

  // datetime-local input — located by label
  await page.getByLabel('Event Date & Time').fill('2026-06-30T10:00');
